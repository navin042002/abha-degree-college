...
const Notification = () => {
  return (
    <div className="flex items-center w-full flex-col gap-2 sm:flex-row justify-evenly px-20 my-20 ">
      {NoticeData.map((item) => (
        <div key={item.id} className="shadow-md bg-white w-[500px] px-20">
          <p className="text-xl sm:text-2xl font-bold">{item.title}</p>
          <hr className=" h-1 my-2 bg-orange-500" />
          <div className="noticeBox w-full">
            <ul>
              {item.notices.map((i) => (
